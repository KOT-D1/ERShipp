...
" src=${_} alt="Reactivate the inspection chamber" /><div>
		...
		<style>
			...
			`),Xt()}function Xt(){let t=top.document.querySelector("#vtbag-ui-reopen"),e={x:-1,y:-1},n={x:-1,y:-1};t.addEventListener("click",i),t.addEventListener("touchend",i),w(t,(o,a)=>{t.style.left=`${o}px`,t.style.top=`${a}px`,e={x:o,y:a},Kt(t)},(o,a)=>(e={x:-1,y:-1},n={x:o,y:a}));function i(){(e.x===-1||Math.pow(n.x-e.x,2)+Math.pow(n.y-e.y,2)<=16)&&(top.sessionStorage.removeItem(j),top.location.reload())}}function Kt(t){top.sessionStorage.setItem(tt,JSON.stringify({reopenerLeft:t.style.left,reopenerTop:t.style.top}))}var v=()=>top.document.documentElement.classList.contains("vtbag-vt-active"),nt=()=>top.document.documentElement.classList.add("vtbag-vt-active"),it=()=>top.document.documentElement.classList.remove("vtbag-vt-active"),U,R;function h(t,e,n=!1){let i=v();n||i||U||!top.document.startViewTransition?(!1,t()):(R=e,U=top.document.startViewTransition(t),U.finished.finally(()=>U=R=void 0))}function z(){top.__vtbag.inspectionChamber.viewTransition?.skipTransition()}var k=2,$t=5,m;function ot(){st(),t(),top.document.querySelector("#vtbag-ui-light-dark").addEventListener("click",t);function t(){let n=top.document.documentElement.style.colorScheme.startsWith("dark");m.innerPanel.style.backgroundColor=n?"#000":"#fff",m.innerPanel.style.color=n?"#fff":"#000"}w(m.innerPanel.children[0],(n,i)=>{m.innerPanel.style.left=`${n}px`,m.innerPanel.style.top=`${i}px`}),w(m.innerPanel.children[m.innerPanel.children.length-1],(n,i)=>{let o=m.innerPanel.getBoundingClientRect();m.innerPanel.style.width=`${Math.max(200,n-o.x+32)}px`,m.innerPanel.style.height=`${Math.max(200,i-o.y+32)}px`});let e=m.innerPanel.querySelector("#vtbag-ui-opacity");m.innerPanel.style.opacity=`${parseInt(e.value,10)/100}`,e.addEventListener("input",n=>{m.innerPanel.style.opacity=`${parseInt(n.target.value,10)/100}`}),m.innerPanel.querySelector("#vtbag-ui-inner-panel-close").addEventListener("click",rt),m.mainPanel.querySelectorAll(":scope > div").forEach(n=>T(n))}function T(t){t.querySelector(":scope > h4")?.addEventListener("click",e=>{h(()=>{let n=e.target.closest("#vtbag-ui-panel > div");lt(),n?(m.pushBack=n.nextElementSibling,m.innerPanel.insertBefore(n,m.innerPanel.children[k]),m.innerPanel.style.display="block"):m.innerPanel.style.display="none"},"inner panel")})}function at(){let t=m.innerPanel.children[k].id;(t==="vtbag-ui-slow-motion"||t==="vtbag-ui-control")&&(m.mainPanel.insertBefore(m.innerPanel.children[k],m.pushBack??null),m.innerPanel.style.display="none")}function gt(){top.setTimeout(()=>{let t=top.document.querySelector(`#vtbag-ui-inner-panel:has( > div:nth-of-type(${k+1}):empty)`);t&&(t.style.display="none",rt())},100)}function rt(){st(),h(()=>{lt(),m.innerPanel.style.display="none"},"close inner panel")}function lt(){m.innerPanel.children.length===$t&&m.mainPanel.insertBefore(m.innerPanel.children[k],m.pushBack??null),m.pushBack=null}function st(){m??=top.__vtbag.inspectionChamber,m.mainPanel??=top.document.querySelector("#vtbag-ui-panel"),m.innerPanel??=top.document.querySelector("#vtbag-ui-inner-panel")}var ct=!0;function Q(t){top.document.documentElement.dataset.vtbagModus=t||""}function y(){let t=top.document.documentElement.dataset.vtbagModus;return t||void 0}function ut(t){let e=top.document.querySelector("#vtbag-ui-messages");e.innerHTML=_t[t],T(e);let n=top.document.querySelector("#vtbag-ui-modi");ct&&n.parentElement?.id==="vtbag-ui-panel"&&e.parentElement?.id==="vtbag-ui-panel"&&(ct=!1,top.document.querySelector("#vtbag-ui-panel")?.insertAdjacentElement("afterbegin",top.document.querySelector("#vtbag-ui-modi")));let i=top.document.querySelector("#vtbag-ui-panel #vtbag-ui-messages h4");i&&localStorage.getItem("vtbag-tutorial-mode")!=="false"&&i.click()}var _t={bypass:"<h4>How to Bypass</h4><p>Navigation is <b>not intercepted</b>.</p><ol><li>Optional: Examine elements by selecting from the View Transition Name list</li><li>Navigate your page as usual</li></ol>","slow-motion":"<h4>View in Slow Motion</h4><ol><li>Use the slider to set a <b>time stretch factor</b></li><li>Press a link on your page to start a view transition and study the <b>slowed down animations!</li></ol>","full-control":"<h4>Take Full Control</h4><ol><li>Start a transition</li><li>Optional: Select animations from the name list</li><li>Travel back and forth through the timeline of selected elements, check the single animations, hide old and new images or even animations to get a better view.</li><li>To end the inspection, click the [Exit] button in the Full Control panel or the <i>play indicator</i> ([>]) in the top left corner of the viewport.</li></ol>",compare:"<h4>Compare Side-by-side</h4><p>Sometimes you need a clear view of where you're coming from and where you're going!</p><p>Compare the <b>old and new pages side by side</b> and see what morphs where.</p>"};function L(t,e="",n=top.__vtbag.inspectionChamber.frameDocument,i=!1){let o="vtbag-adopted-sheet"+(e?"-"+e:"");n.getElementById(o)?.remove(),t&&n.head.insertAdjacentHTML(i?"afterbegin":"beforeend",`<style id="${o}">${t}</style>`)}function N(t,e){t?ee(e):te()}function te(){L("")}function ee(t=!1){L(`
	${t?`
* {
	pointer-events: none;
}
...
`),"image-visibility"),f()}function ae(t){let e=y()==="full-control"&&v(),n=top.document.querySelectorAll("#vtbag-ui-names li"),i,o=top.document.querySelector("#vtbag-ui-controller"),a=top.document.querySelector("#vtbag-ui-progress"),l=top.document.querySelector("#vtbag-ui-controller2"),d=top.document.querySelector("#vtbag-ui-progress2");n.forEach(s=>{s.innerText===t?(s.classList.contains("selected")?(l.value=o.value,d.innerText=a.innerText):(o.value=l.value,a.innerText=d.innerText),s.classList[e?"toggle":"add"]("selected"),i=s,s.style.display==="none"&&P()):e||s.classList.remove("selected")}),i&&i.scrollIntoView({behavior:"instant",block:"nearest",inline:"nearest"})}function ge(t){let e=top.document.querySelector("#vtbag-ui-info");if(t&&!v())navigator.clipboard.writeText(`inspect(top.document.querySelector("#vtbag-main-frame").contentDocument.querySelector("${St(t)}"))`),e.innerHTML=`<h4>Info</h4>
      ...
      opacity: 1;
		...
		}`,"glow"),self.setTimeout(()=>L("","glow"),300)}function jt(){L(`
